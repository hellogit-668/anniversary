...
## 预加载策略
- 所有缩略图在页面打开时立即并行预加载（S2 照片浮页秒开）
- 全尺寸图在 600ms 后开始**串行预加载**（一次一张，加载完等 80ms 再拉下一张，全程只占 1 条连接）
- 故事页 data-src 懒加载作为兜底（进入页才加载），80ms 间隔逐张
- `preloadNext()` 预加载相邻页
- 加图时无需改动预加载逻辑（自动遍历 PHOTOS）

## 调试入口
- URL 加 `?p=N`（N 为照片索引，0-indexed），加载后点击右下角 **⚡按钮** 跳转到该照片故事页
- `?p=0` 第一张、`?p=12` 新增照片第一张
- ⚡按钮默认跳 S2（图片集），方便测试
